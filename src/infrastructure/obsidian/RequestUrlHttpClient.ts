...
export class RequestUrlHttpClient implements HttpClient {
  getJson(url: string, headers: Record<string, string>, signal: AbortSignal): Promise<HttpResponse> {
    return this.send({ url, method: "GET", headers: { Accept: "application/json", ...headers }, throw: false }, signal);
  }

  postJson(url: string, body: unknown, headers: Record<string, string>, signal: AbortSignal): Promise<HttpResponse> {
    return this.send({ url, method: "POST", body: JSON.stringify(body), headers: { "Content-Type": "application/json", ...headers }, throw: false }, signal);
  }

  private async send(req: Parameters<typeof requestUrl>[0], signal: AbortSignal): Promise<HttpResponse> {
    if (signal.aborted) throw abortError();
    const res = requestUrl(req);
